...
**nonebot-plugin-nospam** 是一款（ps：依托alconna，理论上其他平台也能支持）全自动、高智能的防刷屏 NoneBot2 插件。

插件在**滑动时间窗口**内，通过**文本模糊匹配**和**图像感知哈希**识别刷屏行为，可以识别“相似图片/文本”等刷屏方式。检测到刷屏后，插件会自动撤回刷屏消息并进行禁言处理（如果配置）。且使用计数方式统计刷屏，例如设定刷屏阈值为4条，某用户依次发送“1”“1”“2”“1”“1”五条消息，则插件会精确撤回四个“1”，不撤回“2”。

## 🚀 核心特性

- **🧩 适配器兼容**：消息解析与撤回基于 `nonebot-plugin-alconna` `uniseg`，理论上兼容alconna支持的所有适配器。
- **🧠 智能文本检测**：通过文本结构归一化和 Levenshtein 模糊算法相似度计算，防范形变与掺杂无意义字符的文字刷屏。
- **👁️ 视觉查重检测**：通过图像感知计算，提取得出平均哈希、差异哈希、图像边缘与区域签名，识别出经过压缩或尺寸变化后的重复图片同时，防止大体结构相同但内容不同的图片被拦截。
...
只需在配置中添加（白名单模式）或避免排除群组（黑名单模式），并确保机器人在对应群内拥有 **管理员** 或 **群主** 权限。
